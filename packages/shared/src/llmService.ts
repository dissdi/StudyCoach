...
function buildMinuteSystemPrompt(
  ...
  if (flags?.suppressEyeMentions || flags?.readingMode) {
    hardRules.push(
      `[절대 규칙 — 눈/시선/자리비움 관련 일체 언급 금지]
사용자가 종이책·노트를 보는 중이라 카메라에는 시선이 아래로 향한 상태(혹은 얼굴이 잠시 안 잡히는 상태)가 정상입니다.
다음 표현을 절대 사용하지 마세요: "눈 감", "눈 뜨", "눈빛", "눈꺼풀", "눈 개방도", "시선", "졸린", "졸음", "꾸벅", "내려다", "고개 들", "자리 비", "자리비움", "자리를 비", "자리에 앉", "들락", "돌아와", "어디 갔", "어디 있", "뿌리를 내".
얼굴 상태 데이터에 'tired'나 'absent'가 보이더라도 그것은 책을 읽는 자세이지 졸음·자리비움이 아닙니다. 졸음·자리비움 관련 코칭을 절대 하지 마세요.
...
function buildDirectReplySystemPrompt(
  ...
  const eyeRule = (context.suppressEyeMentions || context.readingMode)
    ? `\n[절대 규칙 — 눈/시선/자리비움 관련 일체 언급 금지] 사용자가 책·노트를 보는 중이라 시선이 아래이거나 얼굴이 잠시 안 잡힐 수 있습니다. "눈 감", "눈 뜨", "눈빛", "눈꺼풀", "눈 개방도", "시선", "졸린", "졸음", "꾸벅", "내려다", "고개 들", "자리 비", "자리비움", "자리에 앉", "들락", "돌아와", "어디 갔" 표현을 절대 쓰지 마세요. 졸음·자리비움 추측 금지. 시간·집중·격려·과목 진행만 다루세요.`
    : '';
